...
class TestNewsDetailPage(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.news = News.objects.create(title='Заголовок', text='Текст')
        cls.detail_url = reverse('news:detail', args=(cls.news.id,))
        cls.author = User.objects.create(username='Автор комментариев')
        now = timezone.now()
        ...
        # Явные даты created — иначе оба комментария могут совпасть по времени.
